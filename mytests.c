...
#include <error.h>
#include "lab.h"
#include <pthread.h>                                                                        //TO DO - add ptread library

// function prototypes
int check_if_sorted(int A[], int n);
void generate_random_array(int A[], int n, int seed);
void *parallel_mergesort(thread_data_input *data); //TO DO - add function parallel merge sort
...
int main(int argc, char **argv) {  
                                        ...
                                        //TO DO - Increase the number of armument to add 1 to be depth
	...
	//malloc my data structure and itiliaze it , use arg from user
	// Data for thread 2
       thread_data_input * data;  
		//allocate memory   
		data = (thread_data_input*)malloc(sizeof(thread_data_input)); 
		//initialize data		
        data->left = 1;
        data->right = n;
        data->depth = depth;
